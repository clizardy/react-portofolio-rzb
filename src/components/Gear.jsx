...
const GEAR_ITEMS = [
  {
    ...
    icon: <FaCamera />,
    color: "bg-slate-900 text-white backdrop-blur-md", // Ubah jadi agak transparan
    ...
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-[1001] flex items-center justify-center px-4 py-6 md:py-10">
          ...
          {/* 1. BACKDROP (Lebih transparan biar efek kacanya kelihatan) */}
          <motion.div 
             initial={{ opacity: 0 }} 
             animate={{ opacity: 1 }} 
             exit={{ opacity: 0 }}
          ...
          {/* 2. MODAL CONTAINER (GLASSMORPHISM EXTREME) */}
          <motion.div
            initial={{ scale: 0.9, opacity: 0, y: 20 }}
            animate={{ scale: 1, opacity: 1, y: 0 }}
            exit={{ scale: 0.9, opacity: 0, y: 20 }}
            transition={{ type: "spring", stiffness: 300, damping: 30 }}
            // 👇 INI BAGIAN KUNCI GLASSMORPHISM-NYA
            className="
                ...
                shadow-2xl overflow-hidden flex flex-col backdrop-blur-sm
            "
          >
            
            {/* Header Sticky (Transparan juga) */}
            ...
            {/* Content Scrollable (Background Transparent) */}
            <div className="flex-1 overflow-y-auto p-6 md:p-8 bg-transparent scrollbar-hide">
